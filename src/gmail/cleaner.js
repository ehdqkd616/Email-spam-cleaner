...
async function scanAll(gmailClient, selectedKeys, readFilter = 'is:unread') {
  const results = {};
  for (const key of selectedKeys) {
    ...
    try {
      ...
      spinner.fail(`[${def.name}] 검색 실패: ${err.message}`);
      logger.error('SCAN', `[Gmail][${def.name}] 검색 실패: ${err.message}`, true);
      results[key] = { ...def, query, messages: [], error: err.message };
    }
  }
  return results;
}

async function dryRun(gmailClient, selectedKeys, readFilter) {
  ...
  for (const result of Object.values(results)) {
    ...
    const samples = await gmailClient.getMetadata(result.messages.slice(0, 15).map((m) => m.id));
    spinner.stop();
...
async function execute(gmailClient, selectedKeys, mode = 'trash', readFilter) {
  const results = await scanAll(gmailClient, selectedKeys, readFilter);
  const idSet = new Set();
  ...
  const metadatas = await gmailClient.getMetadata(allIds);
  metaSpinner.stop();
  ...
  try {
    const count = mode === 'trash'
      ? await gmailClient.trashMessages(safeIds)
      : await gmailClient.deleteMessages(safeIds);
    spinner.succeed(
      mode === 'trash'
    ...
    logger[logLevel]('CLEAN', `[Gmail] ${label} 완료 — ${count}개 (보존 제외: ${skipped.length}개)`, true);
    return count;
  } catch (err) {
    logger.error('CLEAN', `[Gmail] ${label} 오류: ${err.message}`, true);
    spinner.fail(`오류 발생: ${err.message}`); throw err;
  }
}

module.exports = { dryRun, execute, scanAll };
